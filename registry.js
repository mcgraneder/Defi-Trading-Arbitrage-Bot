...
async function FindArbitrageOpportunity() {
    ...
    skip != skip;
        ...
        try {
            ...
            uniswapReserve1 = uniswapReserve[1];
            var priceEth = (uniswapReserve0 / uniswapReserve1);
            console.log(uniswapReserve0, uniswapReserve1);
            ...
            var uniAmountOut = await uniswapRouterContract.methods.getAmountOut(amountIn, uniswapReserve1, uniswapReserve0).call()
            // var sushiAmountIn = await uniswapRouterContract.methods.getAmountIn(amountIn, crowswapReserve0, crowswapReserve1).call();

            console.log("\n 1 WETH on Uniswap will get you 0.2103 WETH IN DAI, whereas you can purchase 0.2110 DAI in WETH on sushiswap ")
            var sushiAmountIn = await sushiswapRouterContract.methods.getAmountIn(amountIn, sushiswapReserve0, sushiswapReserve1).call();
            // var quote = await uniswapRouterContract.methods.quote(amountIn, uniswapReserve0, uniswapReserve1).call()
            console.log(uniAmountOut / 10**18, crowAmountIn / 10**18)
            ...
            const sushiDifference = 1/sushiPrice - uniAmountOut/amountIn;
            console.log(sushiDifference)
            ...
            var sushiProfit = (sushiTotalDifference * 0.000201) -  web3.utils.fromWei(gasCost.toString(), "Ether");
