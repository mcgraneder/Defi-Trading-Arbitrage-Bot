...
const integerFromWrappedValueCases = [
    integerFromIntegerValue,
    integerFromEnumValue,
    integerFromDecimalValue
];
const integerCasesBasic = [
    ...
    integerFromEnumError,
    integerFromUdvtValue,
...
function* enumFromNameString(dataType, input, wrapOptions) {
    ...
    const numeric = matchingType
        ? options.indexOf(finalComponent)
...
function* integerFromNumber(dataType, input, wrapOptions) {
    ...
    if (!Number.isSafeInteger(input)) {
        throw new errors_1.TypeMismatchError(dataType, input, wrapOptions.name, 5, "Input number is not a Javascript safe integer");
    }
...
function* integerFromIntegerValue(dataType, input, wrapOptions) {
    if (!Utils.isWrappedResult(input)) {
        throw new errors_1.TypeMismatchError(dataType, input, wrapOptions.name, 1, "Input was not a wrapped result");
    }
    if (input.type.typeClass !== "int" &&
...
function* integerFromDecimalValue(dataType, input, wrapOptions) {
    if (!Utils.isWrappedResult(input)) {
        throw new errors_1.TypeMismatchError(dataType, input, wrapOptions.name, 1, "Input was not a wrapped result");
    }
    if (input.type.typeClass !== "fixed" &&
...
function* integerFromEnumValue(dataType, input, wrapOptions) {
    if (!Utils.isWrappedResult(input)) {
        throw new errors_1.TypeMismatchError(dataType, input, wrapOptions.name, 1, "Input was not a wrapped result");
    }
...
function* integerFromEnumError(dataType, input, wrapOptions) {
    if (!Utils.isWrappedResult(input)) {
        throw new errors_1.TypeMismatchError(dataType, input, wrapOptions.name, 1, "Input was not a wrapped result");
    }
    if (input.type.typeClass !== "enum") {
...
function* integerFromUdvtValue(dataType, input, wrapOptions) {
    if (!Utils.isWrappedResult(input)) {
        throw new errors_1.TypeMismatchError(dataType, input, wrapOptions.name, 1, "Input was not a wrapped result");
    }
...
function validateAndWrap(dataType, asBN, wrapOptions, input) {
